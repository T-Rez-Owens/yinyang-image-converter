...
def brightness_match_images(low_img, top_img, chars1, top_img_chars):
    ...
    target_brightness = (chars1['brightness'] + top_img_chars['brightness']) / 2
    ...
    brightness_factor_top = target_brightness / top_img_chars['brightness']
...
def yin_yang_with_images_rotation_preview(config, lower_image_path, top_image_path, 
                                        unify_method, transforms, rotation_angle):
    """Create a yin-yang for rotation preview with specific top image rotation."""
    
    # Get settings from config
    processing = config.get_processing_settings()
    output_settings = config.get_output_settings()
    
    # Don't show images in rotation preview mode
    ...
    R = processing['radius']
    x = np.linspace(-R*1.2, R*1.2, processing['resolution'])
    y = np.linspace(-R*1.2, R*1.2, processing['resolution'])
    X, Y = np.meshgrid(x, y)
    ...
    result_img = np.zeros((processing['resolution'], processing['resolution'], 3), dtype=np.uint8)
    
    # Resize and transform input images
    low_img_resized = low_img.resize((processing['resolution'], processing['resolution']))
    top_img_resized = top_img.resize((processing['resolution'], processing['resolution']))
    
    # Apply transformations to lower image
    if transforms['low_img_flip_horizontal']:
        low_img_resized = low_img_resized.transpose(Transpose.FLIP_LEFT_RIGHT)
    if transforms['low_img_flip_vertical']:
        low_img_resized = low_img_resized.transpose(Transpose.FLIP_TOP_BOTTOM)
    if transforms['low_img_rotation'] != 0:
        low_img_resized = low_img_resized.rotate(-transforms['low_img_rotation'], expand=True)
    
    # Apply transformations to top image (use passed rotation_angle)
    if transforms['top_img_flip_horizontal']:
        top_img_resized = top_img_resized.transpose(Transpose.FLIP_LEFT_RIGHT)
    if transforms['top_img_flip_vertical']:
        top_img_resized = top_img_resized.transpose(Transpose.FLIP_TOP_BOTTOM)
    if rotation_angle != 0:
        top_img_resized = top_img_resized.rotate(-rotation_angle, expand=True)
    
    # Resize back to target resolution after rotation
    low_img_resized = low_img_resized.resize((processing['resolution'], processing['resolution']))
    top_img_resized = top_img_resized.resize((processing['resolution'], processing['resolution']))
    
    low_img_array = np.array(low_img_resized)
    top_img_array = np.array(top_img_resized)
    
    # Fill regions with appropriate images
    for i in range(processing['resolution']):
        for j in range(processing['resolution']):
    ...
    ax.set_aspect('equal')
    ax.axis('off')
    ax.set_title(f'Top Image Rotation: {rotation_angle}°', fontsize=14, pad=10)
    
    # Save to temp directory
    os.makedirs(output_settings['output_directory'], exist_ok=True)
    output_filename = Path(output_settings['output_directory']) / f"{output_settings['filename_prefix']}_top_rot_{rotation_angle:03d}.png"
...
def yin_yang_with_synchronized_rotation_preview(config, lower_image_path, top_image_path, 
                                              unify_method, transforms, rotation_angle):
    """Create a yin-yang for rotation preview with both images rotated to the same angle."""
    ...
    # Apply transformations to lower image (use synchronized rotation_angle)
    if transforms['low_img_flip_horizontal']:
        low_img_resized = low_img_resized.transpose(Transpose.FLIP_LEFT_RIGHT)
    if transforms['low_img_flip_vertical']:
        low_img_resized = low_img_resized.transpose(Transpose.FLIP_TOP_BOTTOM)
    if rotation_angle != 0:
        low_img_resized = low_img_resized.rotate(-rotation_angle, expand=True)
    
    # Apply transformations to top image (use synchronized rotation_angle)
    if transforms['top_img_flip_horizontal']:
        top_img_resized = top_img_resized.transpose(Transpose.FLIP_LEFT_RIGHT)
    if transforms['top_img_flip_vertical']:
        top_img_resized = top_img_resized.transpose(Transpose.FLIP_TOP_BOTTOM)
    ...
    ax.set_aspect('equal')
    ax.axis('off')
    ax.set_title(f'Both Images Rotated: {rotation_angle}°', fontsize=14, pad=10)
    
    # Save to temp directory with descriptive filename
    os.makedirs(output_settings['output_directory'], exist_ok=True)
    output_filename = Path(output_settings['output_directory']) / f"{output_settings['filename_prefix']}_both_rot_{rotation_angle:03d}.png"
...
def yin_yang_with_methods_preview(config, lower_image_path, top_image_path, unify_method, temp_output_dir):
    """Create a yin-yang for methods comparison in temp_preview folder."""
    ...
    # Don't show images in preview mode
    plt.ioff()  # Turn off interactive mode
    fig, ax = plt.subplots(figsize=(8, 8))
    ...
    ax.set_aspect('equal')
    ax.axis('off')
    ax.set_title(f'Method: {unify_method}', fontsize=14, pad=10)
    
    # Save to temp_preview directory
    os.makedirs(temp_output_dir, exist_ok=True)
    output_filename = Path(temp_output_dir) / f"{output_settings['filename_prefix']}_{unify_method}.png"
...
def main():
    ...
    if processing['mode'] == 'rotation_preview':
        rotations = processing['rotations']
        base_transforms = config.get_transformations()
        
        print(f"\\nGenerating synchronized rotation preview...")
        print(f"Both images rotating together through {len(rotations)} angles")
        print(f"Using method: {processing['method']}")
        print(f"Rotations: {rotations}")
        
        # Generate temp previews
        preview_results = []
        
        for rotation in rotations:
            print(f"\\nProcessing both images at {rotation}°...")
            
            result = yin_yang_with_synchronized_rotation_preview(
                config, low_img_path, top_image_path, 
                processing['method'], base_transforms, rotation
            )
            if result:
                preview_results.append(result)
        
        print(f"\\n✅ Generated {len(preview_results)} synchronized rotation preview images!")
        print(f"📁 Check temp_preview folder to see all rotation options")
        
        # Also generate final image with your specific config settings
        print(f"\\n🎯 Generating final image with your config settings...")
        print(f"   Lower: {base_transforms['low_img_rotation']}° | Top: {base_transforms['top_img_rotation']}°")
        
        final_result = yin_yang_with_images(config, low_img_path, top_image_path, processing['method'])
        
        if final_result:
            print(f"\\n✅ Final yin-yang saved to output folder!")
            print("\\n💡 Compare the temp_preview options with your final result")
            results = preview_results + [final_result]
        else:
            results = preview_results
    else:
        # Methods mode - generate all unification methods for comparison
        methods = ['brightness_match', 'histogram_match', 'color_balance', 'edge_enhance']
        primary_method = processing['method']
        
        print(f"\\nGenerating all unification methods for comparison...")
        print(f"Methods: {methods}")
        print(f"Primary method: {primary_method}")
        
        # Generate all methods in temp_preview for comparison
        method_results = []
        temp_config = config
        ...
        for method in methods:
            ...
            # Create a temp version with temp_preview output directory
            temp_output_dir = "../temp_preview"
            result = yin_yang_with_methods_preview(
                config, low_img_path, top_image_path, method, temp_output_dir
            )
            if result:
                method_results.append(result)
        
        print(f"\\n✅ Generated {len(method_results)} method comparison images in temp_preview!")
        
        # Generate final image with primary method in output folder
        print(f"\\n🎯 Generating final image with primary method: {primary_method}")
        final_result = yin_yang_with_images(config, low_img_path, top_image_path, primary_method)
        
        if final_result:
            print(f"\\n✅ Final yin-yang saved to output folder!")
            print("\\n💡 Compare the method options in temp_preview with your final result")
            results = method_results + [final_result]
        else:
